...
Jump estimation uses weighted average (barycenter) of GPU times:
  1. Compute equalizing_time = weighted avg of T_nv and T_in, weighted by rows/sec
  2. Compute deltaT = difference between slower GPU's time and equalizing_time
  3. Compute ADDED_ROWS = deltaT * rows/sec of the device receiving more rows
  4. Shift the split by ADDED_ROWS
...
def find_optimal_split(R_in_start, nvidia_dev, intel_dev, h_A, h_B):
    ...
    Uses weighted average (barycenter) to estimate the equalizing time,
    then computes how many rows to shift based on deltaT * rows/sec.
    """
    ...
    print(f"\n{'Iter':<6} {'NVIDIA rows':<13} {'Intel rows':<12} {'Wall(s)':<9} "
          f"{'T_nv(s)':<9} {'T_in(s)':<9} {'Eq.Time':<9} {'Rows shift':<10}")
    print("-" * 78)
    ...
    for i in range(MAX_ITERATIONS):
        ...
        # Concurrent rows/sec from this split's profiling
        rate_nv = R_nv / T_nv
        rate_in = R_in / T_in

        # Weighted average (barycenter) to find the equalizing time
        # eq_time is the target where both GPUs should ideally finish
        eq_time = (T_in * rate_in + T_nv * rate_nv) / (rate_in + rate_nv)

        # Determine which GPU is the bottleneck and compute rows to shift
        if T_nv > T_in:
            # NVIDIA is slower — give Intel more rows
            delta_t = T_nv - eq_time
            added_rows = rate_in * delta_t
            jump = snap_to_64(R_in + added_rows) - R_in
        else:
            # Intel is slower — give NVIDIA more rows (reduce Intel)
            delta_t = T_in - eq_time
            removed_rows = rate_nv * delta_t
            jump = R_in - snap_to_64(R_in - removed_rows)
            jump = -jump

        imbalance_ms = abs(T_nv - T_in) * 1000

        print(f"{i+1:<6} {R_nv:<13} {R_in:<12} {wall:<9.3f} "
              f"{T_nv:<9.3f} {T_in:<9.3f} {eq_time:<9.3f} {jump:>+8}")
